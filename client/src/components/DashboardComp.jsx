...
useEffect(()=>{
  ...
  const fetchRequests = async()=>{
    try {
        const res = await fetch('/api/request/getreq?limit=5');
        const data = await res.json();
        if(res.ok)
           {
         ...
         w-full rounded-md shadow-md">
            <div className="flex justify-between ">
                <div className="">
                    ...
                    <FiArrowDownCircle className="bg-purple-600 text-white rounded-full 
                    text-5xl p-3 shadow-lg"/>
            </div>
                <div className="flex gap-2 text-sm">
                    <span className="text-green-500 flex items-center">
  ...
  <Table.Body key={user._id} className="divide-y">
    <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
      ...
      <Table.Cell>
        ...
        rounded-md dark:bg-gray-800">
            <div className="flex justify-between p-3 text-sm font-semibold">
                <h1 className="text-center p-2">Recent Requests</h1>
                <Button outline gradientDuoTone='purpleToPink'>
                  <Link to={'/dashboard?tab=requests'}>See all</Link>
                </Button>
            </div>
            <Table hoverable>
                <Table.Head>
                    <Table.HeadCell>User Email</Table.HeadCell>
                    <Table.HeadCell>GitHub Repository Link</Table.HeadCell>
                </Table.Head>
                {requests && requests.map((requests) => (
  <Table.Body key={requests._id} className="divide-y">
    <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
      {/* Render your table cells (Table.Cell) here */}
      <Table.Cell>
      {requests.em}
